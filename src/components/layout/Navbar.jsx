  ...
  if (!link.children) {
    return (
      <NavLink
        to={link.to}
        className={({ isActive }) =>
          `relative text-sm font-medium transition-colors duration-150 after:absolute after:-bottom-0.5 after:left-0 after:h-[2px] after:w-full after:origin-left after:scale-x-0 after:rounded-full after:bg-[var(--color-primary)] after:transition-transform after:duration-200 hover:after:scale-x-100 ${isActive ? 'after:scale-x-100' : ''}`
        }
          ...
          >
                    ...
                    {col.items.map((item) => (
                      <li key={item.to}>
                        <Link
                          ...
                          className="block text-sm font-medium transition-colors duration-150 hover:text-[color:var(--color-primary)]"
            ...
            >
              <Link
                ...
                onClick={() => setOpen(false)}
                className="text-xs font-semibold uppercase tracking-wide transition-colors hover:opacity-80"
      ...
      >
        ...
        <div className="container relative flex h-16 items-center gap-4">
          ...
          <Link
            to={ROUTES.HOME}
            id="nav-logo"
            className="mr-6 shrink-0 text-2xl font-extrabold tracking-tight"
            style={{ fontFamily: 'var(--font-heading)' }}
            aria-label="VastraHub Home"
          >
            <span style={{ color: 'var(--color-primary)' }}>Vastra</span>
            <span style={{ color: 'var(--color-text)' }}>Hub</span>
          </Link>
